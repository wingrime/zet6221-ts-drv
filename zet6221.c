...
static void  zet_ts_pen_irq_work(struct work_struct *work)
{
	...
	unsigned int x,y,p;
//	if (mutex_trylock(&ts_lock)) {
		ret = zet_i2c_rxdata(buf, 24);
		pr_info("---------------------zte regs-------------------------------\n");
//	pr_info("ts0x: %d %d %d %d \n",buf[3], buf[4] ,buf[5],buf[6]);
		pr_info("ts1x: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",buf[0],buf[1],buf[2],buf[3] \
			,buf[4],buf[5],buf[6],buf[7],buf[8],buf[9]);	
//	pr_info("ts2x: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",buf[20],buf[21],buf[22],buf[23] \
//		,buf[24],buf[25],buf[26],buf[27],buf[28],buf[29]);
//	pr_info("ts3x: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",buf[30],buf[31],buf[32],buf[33] \
//		,buf[34],buf[35],buf[36],buf[37],buf[38],buf[39]);
//	pr_info("ts4x: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",buf[40],buf[41],buf[42],buf[43] \
//		,buf[44],buf[45],buf[46],buf[47],buf[48],buf[49]);
//	pr_info("ts5x: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",buf[50],buf[51],buf[52],buf[53] \
//		,buf[54],buf[55],buf[56],buf[57],buf[58],buf[59]);
//	pr_info("ts6x: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",buf[60],buf[61],buf[62],buf[63] \
//		,buf[64],buf[65],buf[66],buf[67],buf[68],buf[69]);
//
...
//	pen_state = 1;
		...
		if ((buf[1] & 0x80) &&  ((buf[6] & 0xf)==1) ) {
			...
			x = ((unsigned int)((u8)buf[3]>>4)<<8) + (u8)buf[4];
				...
				p = buf[6] & 0xf;
				pr_info("zet: x1 = %d, y1 = %d z = %d ,z2 = %d.\n",x,y,p,((unsigned int)((u8)buf[6]>>4)<<8));
				if (x > 950)
					x = 950;
				if (y > 630)
					y = 630;
				...
				input_report_abs(data->input_dev, ABS_MT_TRACKING_ID, (buf[1] & 0x80) );
...
//	if (!work_pending(&zet_ts->pen_event_work))
//	{
...
static void ts_write_charge_disable_cmd()
{
	pr_info("ts_write_charge_disable\n");
	zet_set_reg( ZET_REG_CHARGE_DISABLE,0 );
}
static void ts_write_charge_enable_cmd()
{	
	zet_set_reg( ZET_REG_CHARGE_ENABLE ,0);
	pr_info("ts_write_charge_enable\n");
}
...
static int zet_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	 ...
	 */
	setup_timer(&zet_ts->pen_release_timer,  zet_pen_release_timer_func, (unsigned long)zet_ts);
	/* main touchevent work */
	INIT_WORK(&zet_ts->pen_event_work, zet_ts_pen_irq_work);
	zet_ts->ts_workqueue = create_singlethread_workqueue(dev_name(&client->dev));
	...
	err = request_irq(SW_INT_IRQNO_PIO, zet_ts_interrupt, IRQF_TRIGGER_FALLING | IRQF_SHARED, "zet_ts", zet_ts);
